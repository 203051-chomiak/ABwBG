...
```{R}
library(ShortRead)
fq_reads1 <- readFastq("C:/Users/magda/OneDrive/Pulpit/ecoli_simulated1_with_adapters.fq")
fq_reads2 <- readFastq("C:/Users/magda/OneDrive/Pulpit/ecoli_simulated2_with_adapters.fq")
```
...
#sprawdzamy czy odczyty przycięte są krótsze od odczytów surowych? - jeżeli odczyt przyciety jest krótszy to funkcja dla danego odczytu zwróci nam TRUE a sum zliczy TRUE i poda liczbę przyciętych odczytów.
#width - informuje nas o długości każdego odczytu
...
**Zadanie do wykonania:** Przytnij odczyty i zanotuj, jaki procent odczytów został zmodyfikowany.

```{r}
#procent zmodyfikowanych odczytów forward
procent1 <- (284123/309440)*100
print(procent1)

#procent zmodyfikowanych odczytów reverse
procent2 <- (273668/309440)*100
print(procent2)
```
...
**Zadanie do wykonania:** Oblicz, jaki procent odczytów został odrzucony podczas filtracji.

```{r}
proc1 <- (248565/284123)*100
print(proc1)

proc2 <-(223516/273668)*100
print(proc2)
```
...
```{R}
   writeFastq(filtered_reads1, "C:/Users/magda/OneDrive/Pulpit/ecoli_simulated1_processed.fq")
   writeFastq(filtered_reads2, "C:/Users/magda/OneDrive/Pulpit/ecoli_simulated2_processed.fq")
```

2.  Wygeneruj nowe raporty QC dla przetworzonych danych:

```{R}
qa_results1 <- qa("C:/Users/magda/OneDrive/Pulpit/ecoli_simulated1_with_adapters.fq", type = "fastq")   
qa_results1_processed <- qa( "C:/Users/magda/OneDrive/Pulpit/ecoli_simulated1_processed.fq", type = "fastq")
report(qa_results1, dest = "C:/Users/magda/OneDrive/Pulpit/QA_report_read1")
report(qa_results1_processed, dest = "C:/Users/magda/OneDrive/Pulpit/QA_report_read1_processed")

qa_results2 <- qa("C:/Users/magda/OneDrive/Pulpit/ecoli_simulated2_with_adapters.fq", type = "fastq")   
qa_results2_processed <- qa("C:/Users/magda/OneDrive/Pulpit/ecoli_simulated2_processed.fq", type = "fastq")
report(qa_results2, dest = "C:/Users/magda/OneDrive/Pulpit/QA_report_read2")
report(qa_results2_processed, dest = "C:/Users/magda/OneDrive/Pulpit/QA_report_read2_processed")
```

**Zadanie do wykonania:** Opisz zmiany w jakości danych po przycinaniu i filtracji.

Dla sekwencji forward:

Przed przetworzeniem - liczba odczytów - 309440 - podobna frekwencja nukleotydówl najwięcej T - bimodalny wykres, prawdopodobnie systemowy błąd odczytu w platformie - po 3 duplikaty - więcej C i G lizebność spada na końcach odczytów drastycznie

Po przetworzeniu - liczba odczytów - 248565 - zmieniła się frekwencja poszczególnych nukleotydów najwięcej T, zmniejszyła się frekwencja C i G na rzecz A - dobra jakość o wartości 35 - zmniejszenie większości duplikatów do dwóch - lieczebność spada bardziej liniowo, nadal mamy "wąsy" - można zaostrzyć parametr k
...
**Zadanie do wykonania:** Porównaj histogramy i opisz, jak przycinanie wpłynęło na długość odczytów.
...
```{r}
print(sum(width(filtered_reads1) < width(trimmed_reads1)))
print(sum(width(filtered_reads2) < width(trimmed_reads2)))
```
...
```{R}
   writeFastq(filtered_reads1, "C:/Users/magda/OneDrive/Pulpit/ecoli_simulated1_final.fq")
   writeFastq(filtered_reads2, "C:/Users/magda/OneDrive/Pulpit/ecoli_simulated2_final.fq")
```

2.  Wygeneruj ostateczne raporty QC:

```{R}
   qa_results1_final <- qa("C:/Users/magda/OneDrive/Pulpit/ecoli_simulated1_final.fq", type = "fastq")
   report(qa_results1_final, dest = "C:/Users/magda/OneDrive/Pulpit/QA_report_read1_final")
   
   qa_results2_final <- qa("C:/Users/magda/OneDrive/Pulpit/ecoli_simulated2_final.fq", type = "fastq")
   report(qa_results2_final, dest = "C:/Users/magda/OneDrive/Pulpit/flashdrive/QA_report_read2_final")
```

3.  Porównaj raporty QC przed i po usunięciu adapterów.

**Zadanie do wykonania:**Opisz zmiany w jakości danych po usunięciu adapterów.
